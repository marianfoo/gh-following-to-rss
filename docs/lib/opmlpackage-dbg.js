const myVersion = "0.4.24",
      myProductName = "opmlPackage";
exports.parse = parse;
...
function parse(opmltext, callback) {
  //returns a JavaScript object with all the info in the opmltext
  //Changes
  ...
  function addGenerator(theOpml) {
    //follow the example of RSS 2.0
    try {
  ...
  xml2js.parseString(opmltext, options, function (err, jstruct) {
    if (err) {
      ...
      if (jstruct == null) {
        //12/27/21 by DW
        let err = {
          message: "Internal error: xml2js.parseString returned null."
        };
        callback(err);
      } else {
        var theOutline = {
...
function stringify(theOutline) {
  //returns the opmltext for the outline
...
function expandInclude(theNode, callback) {
  //1/4/22 by DW
  //Changes
...
function expandIncludes(theOutline, callback) {
  ...
  function expandBody(theBody, callback) {
    var theNewBody = new Object(),
        lastNewNode = theNewBody,
        stack = new Array(),
        currentOutline;
      ...
      function doLevel(head, path, levelcompletecallback) {
        function doOneSub(head, ixsub) {
          if (head.subs !== undefined && ixsub < head.subs.length) {
            var sub = head.subs[ixsub],
                subpath = path + getNameAtt(sub);
              ...
              if (sub.type == "include") {
                ...
                readInclude(sub, function (theIncludedOutline) {
                  if (theIncludedOutline !== undefined) {
                    ...
                    //6/25/15 by DW -- don't let errors derail us
                    doOneSub(head, ixsub + 1);
                  }
                });
              } else {
...
//# sourceMappingURL=opmlpackage.js.map